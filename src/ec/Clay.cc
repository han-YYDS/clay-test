...
Clay::Clay(int n, int k, int w, vector<string> param) { // param = n-1 = 3
    _n = n; // 4
    _k = k; // 2
    _w = w; // 4
//    _opt = opt;

    _d = atoi(param[0].c_str()); // 3 d=n-1 - helpers数目
    _m = _n - _k; // 2
    _q = _d - _k + 1; // 2 组的大小

    if ((_k+_m) % _q) { 
        _nu = _q - (_k+_m)%_q; // 0 shorten 对齐
    } else {
        _nu = 0;
    }
    ...
    _t = (_k+_m+_nu)/_q; // 2 组数 
    _sub_chunk_no = pow_int(_q, _t); // w 
    ...
    for (int i=0; i<_k; i++) {
        for (int j=0; j<_w; j++) { 
            int shortSubPktIdx = i * _w + j; // subpkt的index
...
void Clay::get_erasure_coordinates(vector<int> erased_chunk,
    ...
    for (int idx=0; idx<eclen; idx++) {
        ...
        int x = i%_q; 
        int y = i/_q; 
        erasures[idx] = new Erasure_t(x, y); // 构造中仅填装 x y 坐标
    }
}

void Clay::get_weight_vector(Erasure_t** erasures,
        int* weight_vec) { // 得到每个y截面的 erasure数目
    int i;
...
void Clay::set_planes_sequential_decoding_order(int* order, Erasure_t** erasures) {
    int z, i;
    int z_vec[_t];
    for (z=0; z<_sub_chunk_no; z++) { // 对于4个sub_chunk
        get_plane_vector(z, z_vec); // 在vec中填装
        order[z] = 0;
        for (i=0; i<_m; i++) {
            // 如果 x = zy 
...
ECDAG* Clay::Encode() {
    ...
    // 在这里是将尚未encode生成的节点视为已经erased,对其进行修复等价于对其encode
    vector<int> erased_chunks; // parity [2,3]
    for (int idx=_k+_nu; idx<_n+_nu; idx++) { 
        erased_chunks.push_back(idx); // 填装erased的index 2,3 这里的erased就是待encode的结点
    }

    int i;
    int x, y;
    int hm_w; // 遍历每个有erasure的y截面
    int z, node_xy, node_sw;

    int num_erasures = erased_chunks.size(); // 待encode的paritys
    assert(num_erasures > 0);
    assert(num_erasures == _m); // erasures 应当等于 m = n-k (填装的时候就是填装了n-k个)

    Erasure_t** erasures = (Erasure_t**)calloc(num_erasures, sizeof(Erasure_t*)); // erasures数组
    int weight_vec[_t]; // 每个y截面的erasure数量

    // get the (x, y) coordinate of a node_xy
    get_erasure_coordinates(erased_chunks, erasures); // 在erasures中填装x,y坐标
    cout << "Clay::decode_layered() erasures:" << endl;
    // (0,1) (1,1) 象征着两条parity棱
    ...
    get_weight_vector(erasures, weight_vec); // 根据erasures填装weight_vec,得到每个y截面上的erasures数量 0 2

    cout << "Clay::decode_layered() weight_vector: " ;  // 打印
    for (int i=0; i<_t; i++) cout << weight_vec[i] << " "; // 0 2
    cout << endl;

    // get the number of non-zero bit in the weight vector
    int max_weight = get_hamming_weight(weight_vec);  // 1 具有erasure的y截面个数 
    cout << "Clay::decode_layered() max_weight: " << max_weight << endl; // 1 

    // the number of unpaired symbols in each plane
    int order[_sub_chunk_no]; // 每个z截面的erasure中,不需要耦合的数量
    int z_vec[_t]; // z坐标 t位q进制
    
    // in each plane, get the number of symbols that are unpaired from the node that we should encode
    // 在每个平面中, 得到在erasures中 不需要耦合的symbol数目
    set_planes_sequential_decoding_order(order, erasures); // order = 1,1,1,1


    cout << "Clay::decode_layer() order: ";
    for (int j=0; j<_sub_chunk_no; j++) cout << order[j] << " "; // 1 1 1 1
    cout << endl;

    for (hm_w = 0; hm_w <= max_weight; hm_w++) { // 遍历具有erasure的y截面

        // 耦合 + 编码 
        for (z = 0; z < _sub_chunk_no; z++) { // 遍历z
            // 对于一条x方向的棱
            if (order[z]==hm_w) { // 这个判断属实没看懂 
            ...
            if (order[z] == hm_w) { 
                ...
                for (i = 0; i<num_erasures; i++) {
                    ...
                    if (z_vec[y] == x) { // 解耦copy
                        ...
                        // cout << "239 解耦copy ::old: Join(" << coupled_idx << "; [" << uncoupled_idx << "])" << endl;
                        ecdag->Join(real_coupled_idx, {real_uncoupled_idx}, {1});
                    
                    } else if (z_vec[y] < x) { // 解耦compute
...
void Clay::decode_erasures(vector<int> erased_chunks, int z, ECDAG* ecdag) {
    ...
    get_plane_vector(z,z_vec); // 将z填装成z_vec
    ...
    for (x=0; x < _q; x++) {
        for (y=0; y < _t; y++) {
            ...
            if (find(erased_chunks.begin(), erased_chunks.end(), node_xy) == erased_chunks.end()) {
                if (z_vec[y] < x) { // 耦合compute
                    // this symbol should be paired
                    // 很奇怪,我是认为现在在对其进行耦合,但函数名为什么叫解耦合?
                    get_uncoupled_from_coupled(x, y, z, z_vec, ecdag);
                } else if (z_vec[y] == x) { // 耦合copy
                    ...
                    // cout << "278 耦合copy ::old: Join(" << uncoupled_idx << "; [" << coupled_idx << "])" << endl;
...
vector<int> Clay::get_uncoupled_from_coupled(int x, int y, int z, int* z_vec, ECDAG* ecdag) {
    ...
    // cout << "345 耦合1 ::old: Join(" << idx3 << "; [" << data[0] << ", " << data[1] << "])" << endl;
...
//    cout << "405 解耦1 ::old: Join(" << idx0 << "; [" << data[0] << ", " << data[1] << "])" << endl;
...
//    cout << "417 解耦2 ::old: Join(" << idx1 << "; [" << data[0] << ", " << data[1] << "])" << endl;
...
//                _select_vector, _invert_matrix, 1, _mds_k, _mds_k, _mds_k, 8);
    ...
    //    cout << "605 RS_encode ::old: Join(" << targetidx << "; [" << data[0] << ", " << data[1] << "])" << endl;
